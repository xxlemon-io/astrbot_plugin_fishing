...
## 🔄 Fork版本更新记录 (by Akiyo-dayo)

#### v2.4.0 (2025-11-06) - 骰宝游戏系统完整重构

**🎲 全新骰宝游戏体验**
- **完整游戏机制**：实现多人实时骰宝下注游戏，支持自动结算和智能合并
- **25种下注选项**：
  - 基础下注：大小、单双、豹子 (5种)
  - 指定点数：一点到六点 (6种)
  - 总点数下注：4点到17点 (14种)
- **智能下注系统**：相同类型下注自动合并，避免重复投注，优化用户体验
- **可配置倒计时**：灵活的游戏时间控制，增加游戏紧张感

**🖼️ 双模式显示系统**
- **图片模式**：精美的渐变背景、卡片式布局，提供视觉享受
- **文本模式**：简洁清晰的文字显示，适合不同使用场景
- **管理员控制**：支持实时切换显示模式，满足不同群组需求

**📊 完整帮助与查询系统**
- **骰宝帮助**：详细的游戏规则说明和命令指引
- **骰宝赔率**：完整的赔率表格和概率信息展示
- **游戏状态**：实时查看当前游戏进度和所有玩家下注情况
- **个人下注**：查询个人当前回合的所有下注记录

**⚙️ 高度可配置性**
- **倒计时设置**：管理员可自定义游戏倒计时时间 (30-300秒)
- **显示模式配置**：可设置默认显示模式 (图片/文本)
- **多会话隔离**：支持多个聊天群组同时独立进行游戏

**🎯 用户体验优化**
- **命令趣味化**：所有下注命令使用"鸭"前缀，增加趣味性
- **结算简化**：只显示最终盈亏金额，简洁明了
- **🐟 补充鱼池功能**：新增手动重置剩余稀有鱼数量功能，管理员可使用 `/补充鱼池` 命令重置稀有鱼类库存
- **👤 用户名管理**：新增用户昵称修改功能，用户可使用 `/更新昵称 [新昵称]` 命令自定义显示名称
- **错误处理**：完善的用户友好提示和异常处理机制

**🔧 技术架构升级**
- **独立服务架构**：SicboService提供完整的游戏逻辑封装
- **数据持久化**：完整的数据库事务支持，确保数据一致性
- **图片生成引擎**：基于PIL的高质量图片生成系统
- **多线程安全**：线程安全的游戏状态管理

**📝 完整命令列表**
- 下注命令：`/鸭大` `/鸭小` `/鸭单` `/鸭双` `/鸭豹子` `/鸭一点`~`/鸭六点` `/鸭4点`~`/鸭17点`
- 查询命令：`/骰宝帮助` `/骰宝赔率` `/骰宝状态` `/我的下注`
- 管理命令：`/骰宝模式切换` `/骰宝倒计时设置`
- 用户功能：`/更新昵称 [新昵称]` - 修改用户显示昵称
- 管理功能：`/补充鱼池` - 重置剩余稀有鱼数量 (管理员专用)

> **项目说明**: 本项目基于 [@xxlemon-io](https://github.com/xxlemon-io/astrbot_plugin_fishing) 的优秀项目进行二次开发，感谢原作者的贡献！
