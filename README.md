...
[![Version](https://img.shields.io/badge/Version-2.3.0-brightgreen.svg)](https://github.com/xxlemon-io/astrbot_plugin_fishing/releases/tag/v2.3.0)
...
- **🌩️ 天罚机制**：失败惩罚改为扣除当前金币的0-50%（正态分布），替代固定金币惩罚
  - 轻微天罚（<10%）：约11%概率
  - 中度天罚（10-25%）：约39%概率（最常见）
  - 严重天罚（25-40%）：约38%概率
  - 毁灭性天罚（40-50%）：约12%概率
...
- **成功率机制**：
  - 基础成功率可配置（默认60%）
- **天罚机制**：电鱼失败会遭受天罚，扣除当前金币的0-50%（正态分布）
  - ⚡ 轻微天罚（<10%）：小惩大诫
  - ⚡⚡ 中度天罚（10-25%）：普通惩罚（最常见）
  - ⚡⚡⚡ 严重天罚（25-40%）：损失惨重
  - ⚡⚡⚡⚡ 毁灭性天罚（40-50%）：倾家荡产（极少发生）
- **分段式收益**：根据成功的幸运度分为三个档次
  - ⭐大成功：获得目标鱼塘15%-20%的鱼（小鱼塘20-30条）
  - ✅普通成功：获得目标鱼塘10%-15%的鱼（小鱼塘10-20条）
  - 🔹小成功：获得目标鱼塘5%-10%的鱼（小鱼塘5-10条）
- **高星鱼保护**：最多只能获得一条5星及以上的鱼
- **冷却机制**：电鱼有冷却时间（默认2小时），成功和失败都会触发CD
- **社交互动**：增加玩家间的互动和竞争，富豪和大户更难被电鱼
