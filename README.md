...
#### v2.2.5 (紧急修复：税收重复征收BUG + 多次十连抽卡功能)

- **🚨 修复税收重复执行BUG**：插件重启时不再重复扣税，添加数据库检查确保每日仅执行一次
- **🔧 优化税收线程逻辑**：`is_tax=False` 时不启动线程，增强日志输出（征税用户数、总金币数）
- **🎰 新增多次十连抽卡功能**：支持 `/十连 [卡池ID] [次数]` 命令，自动合并统计结果
- **📊 合并统计优化**：支持1-10星稀有度分布统计，包含物品数量统计和金币总计
...
- 合并统计包含：稀有度分布（1-10星）、物品数量统计、金币总计
- 最多支持100次十连，防止单次操作过多
- 支持显示6-10星超稀有物品的统计
