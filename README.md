...
- **🔧 修复game_config配置缺失**：全面修复main.py中缺失的配置项传递，包括：
  - wheel_of_fate_daily_limit（命运轮盘每日次数）
  - daily_reset_hour（每日重置时间点）
  - user_initial_coins（用户初始金币）
  - market_listing_tax_rate（市场上架税率）
  - tax（税收相关配置）
  - steal_cooldown_seconds（偷鱼冷却时间，修正参数名）
