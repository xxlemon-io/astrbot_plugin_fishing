...
#### v1.7.3 (命令注册与架构重构，不改变游戏逻辑)

- 主入口重构：
  - 统一采用方法装饰器（@filter.command）注册命令，移除类属性式绑定，符合官方规范，确保命令被框架正确扫描与注册。
  - `main.py` 仅作为分发入口，命令实现全部下沉到 `handlers/*`，结构更清晰、可维护性更高。
- 导入与目录：
  - 使用精确导入的仓储与服务实现（`core.repositories.*`、`core.services.*`），移除通配导入，避免潜在的运行时隐患。
- 命令与别名：
  - 新增别名：「我的上架」支持「我的挂单」。
  - 「驱灵」命令的实现函数名调整为 `dispel_protection`（命令名称与用法不变）。
- 向后兼容：
  - 不改变任何业务逻辑与数据结构；现有存档与指令行为保持不变。
...
| `/开启全部钱袋` | `/开全部钱袋`、`/全部钱袋` | 一次性开启所有钱袋类道具 |
...
| `/开启钓鱼后台管理` | - | 启动Web管理后台 |
| `/关闭钓鱼后台管理` | - | 关闭Web管理后台 |
| `/同步道具` | `/管理员 同步道具` | 从 initial_data.py 同步道具模板（管理员） |
| `/代理上线 [用户ID]` | `/login [用户ID]` | 扮演指定用户 |
| `/代理下线` | `/logout` | 结束扮演 |
