...
def up(cursor: sqlite3.Cursor):
    ...
    """
    print("正在执行 018_fix_existing_lock_fields: 修复现有数据的锁定字段...")
    ...
    if 'is_locked' not in user_rods_columns:
        print("user_rods 表缺少 is_locked 字段，正在添加...")
        cursor.execute("""
        ...
        print("user_rods 表已有 is_locked 字段")
    ...
    if 'is_locked' not in user_accessories_columns:
        print("user_accessories 表缺少 is_locked 字段，正在添加...")
        cursor.execute("""
        ...
        print("user_accessories 表已有 is_locked 字段")
        # 确保现有数据的 is_locked 字段不是 NULL
...
def down(cursor: sqlite3.Cursor):
    ...
    """
    print("018_fix_existing_lock_fields 不需要回滚")
    pass
