...
class RefineDestructionShieldEffect(AbstractItemEffect):
    ...
    def apply(
        ...
        """
        charges_to_add = int(payload.get("charges", 1))
        ...
        if existing_buff:
            ...
            new_charges = max(0, current_charges + charges_to_add)
            ...
            new_buff = UserBuff(
                ...
                buff_type=buff_type,
                payload=json.dumps({"charges": charges_to_add, "mode": mode}),
                started_at=get_now(),
                expires_at=None,
            )
            ...
            return {
                "success": True,
                "message": f"🛡 获得精炼护符！可抵消毁坏次数：{charges_to_add}",
            }
