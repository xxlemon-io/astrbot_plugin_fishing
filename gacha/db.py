...
class FishingDB:
            ...
            with self._get_connection() as conn:
                ...
                cursor.execute('''
                    CREATE TABLE IF NOT EXISTS fish (
                        ...
                        description TEXT,
                        rarity INTEGER NOT NULL CHECK (rarity >= 1 AND rarity <= 6),
                ...
                cursor.execute(f'''
                                CREATE TABLE IF NOT EXISTS users (
                                    ...
                                    FOREIGN KEY (equipped_title_id) REFERENCES user_accessories(accessory_instance_id) ON DELETE SET NULL,
        ...
        try:
            with self._get_connection() as conn:
                ...
                fish_data = [
                    ...
                    ('沉没的宝箱', '里面似乎装着什么东西？（打开可能获得水晶或其他物品）', 4, 100, 5000, 15000, None),
                    # Item
                ...
                title_data = [
                    ...
                    (5, '百万富翁', '累计赚取1,000,000水晶', '富有的 {username}'),
                ...
                achievement_data = [
                    ...
                    # 水晶与经济
                    ('水晶猎手', '累计赚取10,000水晶', 'total_coins_earned', 10000, None, 'coins', 1000, 1, 0, None),
                    (
                    '家财万贯', '累计赚取100,000水晶', 'total_coins_earned', 100000, None, 'premium_currency', 20, 1, 0,
                    None),
                    ('富可敌国', '累计赚取1,000,000水晶', 'total_coins_earned', 1000000, None, 'title', 5, 1, 0, None),
                    # 奖励 "百万富翁"

                    # 擦弹相关
                    ('试试手气', '进行第一次擦弹', 'wipe_bomb_profit', 1, None, 'coins', 100, 1, 0, None),  # 随便盈利1块钱就算
                    ('一夜暴富?', '在擦弹中单次盈利超过5000水晶', 'wipe_bomb_profit', 5000, None, 'coins', 2000, 1, 0,
                     None),
                ...
                cursor.execute('''
                    ...
                    VALUES (1, '鱼竿抽奖池', '含有高级鱼竿', 10000, 0)
                ''')
                cursor.execute('''
                    INSERT OR IGNORE INTO gacha_pools (gacha_pool_id, name, description, cost_coins, cost_premium_currency)
                    VALUES (2, '饰品抽奖池', '含有各类钓鱼饰品', 20000, 0)
                ''')
    ...
    def get_user_equipped_title(self, user_id: str) -> Dict:
        """获取用户当前装备的称号信息"""
        with self._get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute("""
                SELECT t.title_id, t.name, t.description, ut.title_instance_id
                FROM user_titles ut
                JOIN titles t ON ut.title_id = t.title_id
    ...
    def equip_title(self, user_id: str, title_instance_id: int) -> bool:
        """装备指定的称号"""
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                ...
                cursor.execute("""
                    SELECT title_id 
                    FROM user_titles 
                    WHERE user_id = ? AND title_instance_id = ?
                """, (user_id, title_instance_id))
                result = cursor.fetchone()
                ...
                cursor.execute("""
                    UPDATE user_titles
                    SET is_equipped = 1
                    WHERE title_instance_id = ?
                """, (title_instance_id,))
                
                # 更新用户表中的装备称号ID
                cursor.execute("""
                    UPDATE users
                    SET equipped_title_instance_id = ?
                    WHERE user_id = ?
                """, (title_instance_id, user_id))
        ...
        try:
            with self._get_connection() as conn:
                ...
                if reward_type == 'coins':
                    ...
                    reward_desc = f"{reward_value * reward_quantity} 水晶"
            ...
            cursor.execute("""
                SELECT gpi.gacha_pool_item_id, gpi.item_type, gpi.item_id, 
                       ...
                       CASE 
                           ...
                           WHEN gpi.item_type = 'coins' THEN '水晶'
                       ...
                       CASE 
                           ...
                           WHEN gpi.item_type = 'coins' THEN '获得一定数量的水晶'
    ...
    def adjust_gacha_pool_weights(self) -> bool:
        ...
        try:
            with self._get_connection() as conn:
                ...
                conn.commit()
                # logger.info("奖池数据已重置，移除了钻石类物品并使用水晶替代")
                return True
    ...
    # 1. 检查用户是否存在
        if not self.is_registered(user_id):
            return {"success": False, "message": "用户未注册，无法更新昵称"}

    # 2. 检查新昵称是否合法（例如非空、无敏感词等）
        if not new_nickname or len(new_nickname.strip()) == 0:
            return {"success": False, "message": "昵称不能为空"}
        if len(new_nickname) > 20:  # 示例：限制昵称长度
            return {"success": False, "message": "昵称最长20个字符"}

    # 3. 调用数据库更新昵称
        success = self.db.update_user_nickname(user_id, new_nickname)
        if success:
            return {"success": True, "message": f"昵称已更新为 {new_nickname}"}
        else:
            return {"success": False, "message": "昵称更新失败，请稍后再试"}    
            ...
            for user in users:
                try:
                    ...
                    if existing_user:
                        ...
                        logger.info(f"更新用户 {user.user_id} 的水晶: +{user.coins}")
                    else:
                        # 如果用户不存在，则插入新用户
                        ...
                        logger.info(f"插入新用户 {user.user_id}, 昵称: {user.nickname}, 水晶: {user.coins}")
