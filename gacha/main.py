...
class FishingPlugin(Star):
    ...
    @filter.command("更新昵称", aliases={"改昵称"})
    async def update_user_nickname(self, event):
        """更新用户昵称"""
        try:
            # 获取消息内容并分割
            parts = event.raw_message.split(maxsplit=1)
            if len(parts) < 2:
                return "❌ 请输入新昵称！格式：更新昵称 新昵称"
        
            new_nickname = parts[1].strip()
            user_id = event.user_id
        
            # 调用业务逻辑
            result = self.FishingService.update_nickname(user_id, new_nickname)
            return result["message"]
    
        except Exception as e:
            return f"❌ 更新昵称时出错：{str(e)}"
    ...
    async def go_fishing(self, event: AstrMessageEvent):
        ...
        if equipped_rod and equipped_rod.get("name") == "海洋之心":
            ...
            last_fishing_time = max(0, last_fishing_time - 60)
            logger.info(f"用户 {user_id} 装备了海洋之心，{last_fishing_time}")
        logger.info(f"用户 {user_id} 上次钓鱼时间: {last_fishing_time}, 当前时间: {current_time}")
        # 3分钟CD (180秒)
        if last_fishing_time > 0 and current_time - last_fishing_time < 300:
            remaining_seconds = int(300 - (current_time - last_fishing_time))
    ...
    async def show_inventory(self, event: AstrMessageEvent):
        ...
        for rarity in sorted(fishes_by_rarity.keys(), reverse=True):
            ...
            for fish in fishes_by_rarity[rarity]:
                message += f"- {fish.get('name')} x{fish.get('quantity')} ({fish.get('base_value', 0)}水晶/个)\n"

        message += f"\n💰 总价值: {total_value}{get_coins_name()}"

        yield event.plain_result(message)

    @filter.command("钓鱼签到", alias={"signin"})  # ok
    ...
    async def view_gacha_pool(self, event: AstrMessageEvent):
        ...
        try:
            ...
            message += f"抽取花费: {pool_details.get('cost_coins', 0)}{get_coins_name()}\n\n"
    ...
    async def show_help(self, event: AstrMessageEvent):
        """显示钓鱼游戏帮助信息"""
        prefix = """前言：使用/注册指令即可开始，鱼饵是一次性的（每次钓鱼随机使用），可以一次买多个鱼饵例如：/购买鱼饵 3 200。鱼竿购买后可以通过/鱼竿查看，如果你嫌钓鱼慢，可以玩玩/擦弹 水晶数量，随机获得0-5倍收益"""
        message = f"""【🎣 钓鱼系统帮助】
    📋 基础命令:
     - /注册: 注册钓鱼用户
     - /钓鱼: 进行一次钓鱼(消耗50{get_coins_name()}，5分钟CD)
     - /钓鱼签到: 每日签到领取奖励
     - /水晶: 查看当前{get_coins_name()}
    ...
    🔧 其他功能:
     ...
     - /排行榜: 查看钓鱼排行榜
     - /鱼类图鉴: 查看所有鱼的详细信息
     - /擦弹 [水晶数]: 向公共奖池投入{get_coins_name()}，获得随机倍数回报（0-5倍）
     - /擦弹历史： 查看擦弹历史记录
     - /查看称号: 查看已获得的称号
     - /查看成就: 查看可达成的成就
     - /钓鱼记录: 查看最近的钓鱼记录
    ...
    async def show_fish_catalog(self, event: AstrMessageEvent):
        ...
        for rarity in sorted(fishes_by_rarity.keys(), reverse=True):
            ...
            for fish in fish_list:
                message += f"- {fish['fish_id']}{fish['name']} (💰 价值: {fish['base_value']}水晶)\n"
                if fish['description']:
    ...
    async def do_wipe_bomb(self, event: AstrMessageEvent):
        ...
        if len(args) < 2:
            yield event.plain_result("💸 请指定要投入的水晶数量，例如：擦弹 100")
            return

        try:
            amount = int(args[1])
            if amount <= 0:
                yield event.plain_result("⚠️ 投入水晶必须大于0")
                return
            ...
            yield event.plain_result("⚠️ 请输入有效的水晶数量")
    ...
    async def show_titles(self, event: AstrMessageEvent):
        ...
        # 获取用户称号
        titles = self.FishingService.get_user_titles(user_id)

        if not titles["success"]:
            yield event.plain_result(titles["message"])
            return

        user_titles = titles["titles"]

        if not user_titles:
            yield event.plain_result("📝 你没有任何称号，加油获取")
            return

            # 正常显示包括已装备状态
        equipment_info = self.FishingService.get_user_titles(user_id)
        equipped_title = equipment_info.get("title")
        equipped_title_id = equipped_title.get("title_instance_id") if equipped_title else None

        # 构建消息
        message = "【📝 称号背包】\n\n"

        for title in user_titles:
            title_instance_id = title.get("title_instance_id")
            is_equipped = title_instance_id == equipped_title_id or title.get("is_equipped", False)

            message += f"ID:{title_instance_id} - {title['name']}"
            if is_equipped:
                message += " [已装备]"
            message += "\n"

            if title["description"]:
                message += f"  📝 描述: {title['description']}\n"
    ...
    async def show_achievements(self, event: AstrMessageEvent):
        ...
        try:
            ...
            message += "💡 提示：完成成就可以获得各种奖励，包括水晶、称号、特殊物品等！"
    ...
    @filter.command("使用称号", alias={"usetitle"})
    async def use_title(self, event: AstrMessageEvent):
        ...
        if len(args) < 2:
            yield event.plain_result("⚠️ 请指定要装备的称号ID")
            return

        try:
            title_id = int(args[1])
            result = self.FishingService.equip_title(user_id, title_id)

            # 增加表情符号
            original_message = result.get("message", "装备称号失败！")
            if "成功" in original_message:
                message = f"📝 {original_message}"
            else:
                message = f"❌ {original_message}"
            ...
            yield event.plain_result("⚠️ 请输入有效的称号ID")

    @filter.permission_type(PermissionType.ADMIN)
    @filter.command("增加水晶", alias={"addcoins"})
    async def add_coins(self, event: AstrMessageEvent):
        """给指定用户增加水晶（管理员命令）"""
        args = event.message_str.split(' ')
        
        if len(args) < 3:
            yield event.plain_result("⚠️ 请使用正确的格式：增加水晶 <用户ID> <水晶数量>")
            return
        ...
        try:
            ...
            if amount <= 0:
                yield event.plain_result("⚠️ 水晶数量必须大于0")
                return
