...
class FishingService:
    ...
    def fish(self, user_id: str, is_auto: bool = False) -> Dict:
        ...
        if is_auto:
            ...
            if not self.db.update_user_coins(user_id, -fishing_cost):
                return {"success": False, "message": "水晶不足，无法进行自动钓鱼"}
    ...
    def sell_all_fish(self, user_id: str) -> Dict:
        ...
        return {"success": True, "message": f"已卖出所有鱼，获得 {total_value} 水晶"}
    ...
    def sell_fish_by_rarity(self, user_id: str, rarity: int) -> Dict:
        ...
        # 验证稀有度参数
        if not (1 <= rarity <= 6):
            return {"success": False, "message": "无效的稀有度，应为1-6之间的整数"}
        ...
        return {"success": True, "message": f"已卖出稀有度为 {rarity} 的鱼，获得 {total_value} 水晶"}
    ...
    def buy_bait(self, user_id: str, bait_id: int, quantity: int = 1) -> Dict:
        ...
        if user_coins < total_cost:
            return {"success": False, "message": f"水晶不足，需要 {total_cost} 水晶"}
    ...
    def buy_rod(self, user_id: str, rod_id: int) -> Dict:
        ...
        if user_coins < rod['purchase_cost']:
            return {"success": False, "message": f"水晶不足，需要 {rod['purchase_cost']} 水晶"}
    ...
    def multi_gacha(self, user_id: str, pool_id: int, count: int = 10) -> Dict:
        ...
        user_coins = self.db.get_user_coins(user_id)
        if user_coins < cost:
            return {"success": False, "message": f"水晶不足，需要 {cost} 水晶"}
    ...
    def _perform_single_gacha(self, user_id: str, pool_id: int) -> Dict:
        ...
        if not self.db.update_user_coins(user_id, -cost):
            return {"success": False, "message": "扣除水晶失败"}
        ...
        if item_type == 'rod':
            ...
            item_info = self.db.get_title_info(item_id)    
        elif item_type == 'coins':
            item_info = {'name': '水晶', 'rarity': 1}
                    ...
                    for user_id in auto_fishing_users:
                        try:
                            ...
                            if equipped_accessory and equipped_accessory.get('name') == "海洋之心":
                                # 海洋之心效果：减少CD时间
                                last_time -= 60  # 减少2分钟CD
                            if current_time - last_time < 300:  # 3分钟CD
                            ...
                            if user_coins < self.get_fishing_cost():
                                ...
                                self.LOG.info(f"用户 {user_id} 水晶不足，已关闭自动钓鱼")
                                continue
    ...
    def set_user_auto_fishing(self, user_id: str, status: bool) -> Dict:
        ...
        if status:
            user_coins = self.db.get_user_coins(user_id)
            if user_coins < self.get_fishing_cost():
                return {"success": False, "message": "水晶不足，无法开启自动钓鱼"}
    ...
    def get_fish_pond(self, user_id: str) -> Dict:
        ...
        return {
            "success": True,
            "message": f"你的鱼塘里有 {stats.get('total_count', 0)} 条鱼，总价值: {stats.get('total_value', 0)} 水晶",
            "stats": stats,
    ...
    def daily_sign_in(self, user_id: str) -> Dict:
        ...
        if self.db.record_daily_sign_in(user_id, coins_reward):
            ...
            result = {
                "success": True,
                "message": f"签到成功！获得 {coins_reward} 水晶",
            ...
            if consecutive_days in [7, 14, 30, 60, 90, 180, 365]:
                ...
                result["message"] += f"，连续签到 {consecutive_days} 天，额外奖励 {bonus_coins} 水晶！"
    ...
    def equip_title(self, user_id: str, title_instance_id: int) -> Dict:
        """装备指定的称号"""
        error = self._check_registered_or_return(user_id)
        if error:
            return error
        ...
        # 检查称号是否存在并属于用户
        if self.db.equip_title(user_id, title_instance_id):
            # 获取称号信息
            title = self.db.get_user_equipped_title(user_id)
            if title:
                return {
                    "success": True,
                    "message": f"成功装备【{title['name']}】！",
                    "title": title
                }
            else:
                return {
                    "success": True,
                    "message": "称号已装备，但无法获取详细信息"
                }
        else:
            return {
                "success": False,
                "message": "装备称号失败，请确认该称号属于你"
            }
    ...
    def get_user_equipped_title(self, user_id: str) -> Dict:
        ...
        title = self.db.get_user_equipped_title(user_id)
        if not title:
            return {"success": True, "title": None}
    ...
    def perform_wipe_bomb(self, user_id: str, contribution_amount: int) -> Dict:
        ...
        if user_coins < contribution_amount:
            return {"success": False, "message": f"水晶不足，当前拥有 {user_coins} 水晶"}
        ...
        # 定义倍数区间和对应的权重
        ranges = [
            (0.0, 0.5, 45),    # 0.0-0.5倍，权重35
            (0.5, 1.0, 35),    # 0.5-1.0倍，权重25
            (1.0, 1.0, 10),    # 1.0-2.0倍，权重20
            (1.0, 1.5, 4),    # 2.0-3.0倍，权重10
            (1.5, 2.5, 3),     # 3.0-5.0倍，权重7
            (2.5, 4.0, 2),     # 5.0-8.0倍，权重2
            (4.0, 5.0, 1),    # 8.0-10.0倍，权重1
        ]
        ...
        return {
            "success": True,
            "message": f"擦弹结果：投入 {contribution_amount} 水晶，获得 {reward_multiplier}倍 奖励，共 {reward_amount} 水晶，{profit_text}！今天还可以擦弹 {remaining} 次。",
