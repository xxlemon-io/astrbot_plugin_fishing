...
async def modify_coins(self: "FishingPlugin", event: AstrMessageEvent):
    """修改用户金币"""
...
async def modify_premium(self: "FishingPlugin", event: AstrMessageEvent):
    """修改用户高级货币"""
    args = event.message_str.split(" ")
...
async def reward_premium(self: "FishingPlugin", event: AstrMessageEvent):
    """奖励用户高级货币"""
    args = event.message_str.split(" ")
...
async def deduct_premium(self: "FishingPlugin", event: AstrMessageEvent):
    """扣除用户高级货币"""
    args = event.message_str.split(" ")
...
async def reward_all_coins(self: "FishingPlugin", event: AstrMessageEvent):
    """给所有注册用户发放金币"""
    args = event.message_str.split(" ")
    if len(args) < 2:
...
async def reward_all_premium(self: "FishingPlugin", event: AstrMessageEvent):
    """给所有注册用户发放高级货币"""
    args = event.message_str.split(" ")
    if len(args) < 2:
        yield event.plain_result(
...
async def deduct_all_coins(self: "FishingPlugin", event: AstrMessageEvent):
    """从所有注册用户扣除金币（不低于0）"""
    args = event.message_str.split(" ")
    if len(args) < 2:
...
async def deduct_all_premium(self: "FishingPlugin", event: AstrMessageEvent):
    """从所有注册用户扣除高级货币（不低于0）"""
    args = event.message_str.split(" ")
    if len(args) < 2:
        yield event.plain_result(
...
async def reward_coins(self: "FishingPlugin", event: AstrMessageEvent):
    """奖励用户金币"""
    args = event.message_str.split(" ")
    ...
    if (current_coins := self.user_service.get_user_currency(target_user_id)) is None:
        yield event.plain_result("❌ 用户不存在或未注册，请检查后重试。")
        return
    if result := self.user_service.modify_user_coins(
...
async def deduct_coins(self: "FishingPlugin", event: AstrMessageEvent):
    """扣除用户金币"""
    args = event.message_str.split(" ")
...
async def start_admin(self: "FishingPlugin", event: AstrMessageEvent):
...
async def stop_admin(self: "FishingPlugin", event: AstrMessageEvent):
    """关闭钓鱼后台管理"""
    if (
        not hasattr(self, "web_admin_task")
        or not self.web_admin_task
        or self.web_admin_task.done()
    ):
...
async def sync_initial_data(self: "FishingPlugin", event: AstrMessageEvent):
...
async def impersonate_start(self: "FishingPlugin", event: AstrMessageEvent):
    """管理员开始扮演一名用户。"""
...
async def impersonate_stop(self: "FishingPlugin", event: AstrMessageEvent):
    """管理员结束扮演用户。"""
    admin_id = event.get_sender_id()
    if admin_id in self.impersonation_map:
...
async def reward_all_items(self: "FishingPlugin", event: AstrMessageEvent):
    """给所有注册用户发放道具"""
    args = event.message_str.split(" ")
    if len(args) < 4:
        yield event.plain_result(
    ...
    for user_id in user_ids:
        try:
            result = self.user_service.add_item_to_user_inventory(
