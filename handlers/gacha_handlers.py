...
def register_gacha_handlers(plugin):
    @filter.command("抽卡", alias={"抽奖"})
    async def gacha(event: AstrMessageEvent):
        """抽卡"""
        ...
        if not pool_id.isdigit():
            ...
            return
        pool_id = int(pool_id)
        result = plugin.gacha_service.perform_draw(user_id, pool_id, num_draws=1)
        if result:
            if result["success"]:
    ...
    @filter.command("十连")
    async def ten_gacha(event: AstrMessageEvent):
        """十连抽卡"""
        user_id = plugin._get_effective_user_id(event)
        args = event.message_str.split(" ")
        if len(args) < 2:
        ...
        if not pool_id.isdigit():
            ...
            return
        pool_id = int(pool_id)
        result = plugin.gacha_service.perform_draw(user_id, pool_id, num_draws=10)
        if result:
            if result["success"]:
                items = result.get("results", [])
    ...
    @filter.command("查看卡池")
    async def view_gacha_pool(event: AstrMessageEvent):
        """查看当前卡池"""
        args = event.message_str.split(" ")
        if len(args) < 2:
        ...
        if not pool_id.isdigit():
            ...
            return
        pool_id = int(pool_id)
        result = plugin.gacha_service.get_pool_details(pool_id)
        if result:
            if result["success"]:
    ...
    @filter.command("抽卡记录")
    async def gacha_history(event: AstrMessageEvent):
        """查看抽卡记录"""
        user_id = plugin._get_effective_user_id(event)
        result = plugin.gacha_service.get_user_gacha_history(user_id)
        if result:
            if result["success"]:
    ...
    @filter.command("擦弹")
    async def wipe_bomb(event: AstrMessageEvent):
        """擦弹功能"""
        user_id = plugin._get_effective_user_id(event)
        args = event.message_str.split(" ")
        if len(args) < 2:
        ...
        if not contribution_amount.isdigit():
            ...
            return
        result = plugin.game_mechanics_service.perform_wipe_bomb(user_id, int(contribution_amount))
        if result:
            if result["success"]:
                message = ""
    ...
    async def wipe_bomb_history(event: AstrMessageEvent):
        """查看擦弹记录"""
        user_id = plugin._get_effective_user_id(event)
        result = plugin.game_mechanics_service.get_wipe_bomb_history(user_id)
        if result:
            if result["success"]:
